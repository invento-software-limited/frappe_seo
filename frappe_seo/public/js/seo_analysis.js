...
frappe_seo.analysis.SEOAnalyzer = class {
	...
	get_data() {
		return {
			...
			content:
				this.frm.doc.content ||
				this.frm.doc.main_section ||
				this.get_builder_content() ||
				"",
		};
	}
	...
	calculate_score(data) {
		...
		if (kw) {
			...
			checks.push({
				...
				status: inDesc ? "success" : "danger",
			});
			if (inDesc) score += 25;

			checks.push({
				label: "Keyphrase in Content",
				status: inContent ? "success" : "danger",
			});
		...
		const titleStatus =
			titleLen >= 40 && titleLen <= 60 ? "success" : titleLen > 0 ? "warning" : "danger";
		checks.push({ label: `Title length (${titleLen} chars)`, status: titleStatus });
		if (titleStatus === "success") score += 15;

		const descLen = data.description.length;
		const descStatus =
			descLen >= 120 && descLen <= 160 ? "success" : descLen > 0 ? "warning" : "danger";
