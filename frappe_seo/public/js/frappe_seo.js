...
function runChecks(frm, fields) {
	...
	if (wordCount < 300) {
		checks.push({
			label: "Text Length",
			status: "ok",
			message: `Contains ${wordCount} words. Aim for 300+ for better ranking.`,
		});
	} else {
		checks.push({
			label: "Text Length",
	...
	if (!desc) {
		...
		checks.push({
			label: "Meta Description",
			status: "ok",
			message: `Length is ${desc.length}. Aim for 120-160 chars.`,
		});
	} else {
		checks.push({
			label: "Meta Description",
			status: "good",
			message: "Length is perfect. ✓",
		});
	}
	...
	if (!title) {
		...
		checks.push({
			label: "Meta Title",
			status: "ok",
			message: "Title too long (aim for < 60 chars).",
		});
	} else {
	...
	if (keyphrase) {
		if (title.toLowerCase().includes(keyphrase)) {
			checks.push({
				...
				status: "good",
				message: "Found in title. ✓",
			});
		} else {
			checks.push({
				label: "Keyphrase in Title",
				status: "bad",
				message: "Not found in title.",
			});
		}
		...
		if (desc.toLowerCase().includes(keyphrase)) {
			checks.push({
				...
				status: "good",
				message: "Found in description. ✓",
			});
		} else {
			checks.push({
				label: "Keyphrase in Description",
				status: "ok",
				message: "Consider adding it to description.",
			});
		}
		...
		const count = (
			content
				.toLowerCase()
				.match(new RegExp(keyphrase.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "gi")) || []
		).length;
		...
		if (count === 0) {
			...
			checks.push({
				label: "Keyphrase Density",
				status: "good",
				message: `${density}% density (${count} times). ✓`,
			});
		}
	}
...
function calcScore(checks) {
	...
	const points = checks.reduce(
		(acc, c) => acc + (c.status === "good" ? 10 : c.status === "ok" ? 5 : 0),
		0
	);
	...
	<div style="display:flex; align-items:center; gap:12px; margin-bottom:16px; padding:14px; background:#f8fafc; border-radius:10px; border:1px solid #e2e8f0;">
		<div style="position:relative; width:50px; height:50px;">
			<svg viewBox="0 0 36 36" style="width:50px;height:50px;transform:rotate(-90deg)">
				...
				<circle cx="18" cy="18" r="15.9" fill="none" stroke="${color}" stroke-width="3" stroke-dasharray="${score} ${
		100 - score
	}" />
			</svg>
		...
		<div>
			...
			<div style="font-size:12px; color:#64748b;">${checks.filter((c) => c.status === "good").length}/${
		checks.length
	} checks passed</div>
		</div>
	</div>
	...
	<div style="display:flex; flex-direction:column; gap:6px; margin-bottom:16px;">
		...
		<div style="display:flex; align-items:center; gap:10px; padding:8px 12px; background:#fff; border:1px solid #e2e8f0; border-radius:8px;">
			<span style="width:8px; height:8px; border-radius:50%; background:${
				c.status === "good" ? "#22c55e" : c.status === "ok" ? "#f59e0b" : "#ef4444"
			};"></span>
			<div style="font-size:12px; font-weight:500;">${
				c.label
			}: <span style="font-weight:400; color:#64748b;">${c.message}</span></div>
		</div>`
			)
			.join("")}
	</div>`;
